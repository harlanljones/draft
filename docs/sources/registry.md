...
|---|---|---|---|
| EADA survey (U.S. Dept. of Education) | Available — U.S. federal public domain | Program-resource context features only (via `draft_model.ingest.eada`) | No blockers: federal work is public domain. Not a player cohort, performance predictor, or outcome source; conservative availability is one release cycle behind. First-party ingest from ope.ed.gov via `draft-model ingest-eada` (manifest with SHA-256 + retrieval dates under `data/empirical/`, gitignored); packaged ncaa_bbStats extract is fallback only. |
